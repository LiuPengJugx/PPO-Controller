...
class EnvMaskCurve(gym.Env):
    ...
    def __init__(self, wLoad):
        ...
        self.temp_affinity_sel_matrix = self.w.compute_affinity_matrix_consider_selectivity(self.time_point + 1,
                                                                                            self.time_point + 2)
        self.temp_workload = []
    ...
    def step(self, action):
        ...
        self.temp_affinity_sel_matrix = self.w.update_affinity_matrix_consider_selectivity(
            self.temp_affinity_sel_matrix, self.time_point + 1)
        ...
        if action == 1:
            ...
            new_par_schema = self.model.partitioner(self.current_affinity_matrix, self.temp_workload,
                                                    self.w.attrs_length)
            operation_cost = DiskIo.compute_repartitioning_cost(self.cur_par_schema, new_par_schema,
                                                                self.w.attrs_length)
            reward, io_cost = self._get_reward(self.cur_par_schema, new_par_schema, self.temp_workload, operation_cost,
                                               time_diff)
            # print(reward)
            if reward >= self.reward_threshold:
                ...
                if reward >= -self.reward_threshold:
                    ...
                    if self.adjust_times >= self.adjust_range_threshold:
                        self.io_cost.append(io_cost)
                        self.last_time_point = self.time_point
                        self.temp_affinity_sel_matrix = self.w.compute_affinity_matrix_consider_selectivity(
                            self.time_point + 1, self.time_point + 1 + 1)
                        state = self._get_state(self.temp_affinity_sel_matrix)
                        self.current_affinity_matrix = self.w.compute_affinity_matrix(0, 0)
                        self.temp_workload = []
                        self.adjust_times = 0
    ...
    def _get_reward(self, old_par_schema, new_par_schema, temp_workload, operation_cost, time_diff):
        ...
        time_diff=math.exp((time_diff-1)/5)
        # if time_diff<=5:
        #     # time_diff=1+(time_diff-1)*0.4
        #     pass
        # elif time_diff>=50:
        #     time_diff = 50 + (time_diff - 50) * 10
