...
class EnvParMask(gym.Env):
    ...
    def __init__(self,wLoad):
        ...
        self.state=self._get_state(self.w.load_sql_by_time_range(self.time_point+1, self.time_point+1+1))
    ...
    def step(self,action):
        ...
        state=self._get_state(self.w.load_sql_by_time_range(self.last_time_point+1, self.time_point + 1+1))
        ...
        if action==1:
            ...
            if reward>=self.reward_threshold:
                ...
                state = self._get_state(self.w.load_sql_by_time_range(self.time_point+1, self.time_point+1+1))
                self.current_affinity_matrix = self.w.compute_affinity_matrix(0, 0)
                self.temp_workload = []
                self.adjust_times = 0
    ...
    def _get_state(self,sqls):
        return self.w.mask_par_matrix(sqls,self.cur_par_schema)
